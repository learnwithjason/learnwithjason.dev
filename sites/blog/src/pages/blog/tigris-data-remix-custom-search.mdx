...
meta:
  title: Build Custom, Faceted Search for Your App with Remix and Tigris Data
  description: >
    Great search experiences are complicated: text search, filters, URL-addressable results, and more. Learn how to implement a full-featured search functionality using Remix and Tigris Data in this tutorial.
...
If you learn better with video and/or want to watch me build this app, I have a video version up on YouTube.
...
Many companies would benefit from implementing a great directory search. **Whether you're working on an e-commerce store, a social media app, an aggregator service, collections of content, or any other approach that consists of interconnected entries, allowing users a way to quickly filter and search is a great way to improve their experience.**

However, it can be really challenging to build out a directory feature that works well. It always starts simple, but the feature requirements pile up fast:

- Display all listings in an easy-to-scan view
- Provide free text search to help users find exactly what they're looking for
- Add clickable filters to narrow down the results based on what the user is interested in
- Make filtered results URL-addressable so they can be bookmarked and shared

Fortunately for us, there are a lot of companies out there working to handle implementations like these so we don't have to.

The team over at [Tigris Data](https://www.tigrisdata.com/) has been working hard to provide an excellent faceted search experience as a built-in component of their data store, and they reached out to me to partner on building out a demo of how we can use that faceted search query feature in conjunction with [Remix](http://remix.run/)'s features to build out a full-featured directory search without needing weeks of dev time.

**In fact, with Tigris and Remix you can build out the whole search functionality from scratch in under an hour.** In this tutorial, we'll walk through the whole process of setting up a new Tigris collection, querying the data, and adding the UX features in Remix to make it all accessible to your users.
...
<Aside>
  **Note:** you _do not_ need this repo to follow along, but you'll need to
  ...
  data layer and building out the logic to use the data in the app.
</Aside>
...
> **A quick note on the app architecture:** If you've worked with Remix before, you might be wondering why this app doesn't have a `routes` directory. Because the URLs in this app are _only_ used for filtering, we want to use the same components and logic for both the root route (`/`) and any sub-routes in the app. Rather than having an [index route](http://remix.run/docs/en/main/guides/routing#index-routes) and a [splat route](http://remix.run/docs/en/main/guides/routing#splats), we're instead using `remix.config.js` to set `app/index.tsx` as the route for all requests to this app.
