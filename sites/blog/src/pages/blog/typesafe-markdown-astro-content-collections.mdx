...
If you prefer learning by watching something get built, I recorded building the demo app in this post as a video tutorial. [Learn about Astro content collections and typesafe Markdown on YouTube.](https://youtu.be/gqz0d1GguTg)

<Figure aspectRatio={9 / 16}>
  <iframe
    width="560"
    height="315"
    src="https://www.youtube-nocookie.com/embed/gqz0d1GguTg"
...
**Heads up!** Because this post isn't about the structure or styling of a blog, we won't cover how the HTML or CSS works in this project. If you have questions, definitely [let me know](https://jason.af/links)!
...
- Publish date
- Title
- Slug — this is optional, but if we want to customize the URL of the blog, we'd use this
...
The `slug` property at the top level of the collection config is a special helper Astro provides to allow us to customize the URL of a blog post. By default, the file name is used, but if you prefer, you can set a custom frontmatter fields and use the `slug` helper to replace the default. In this case, we look for a frontmatter field called `slug` and fall back to the default if one isn't provided.
...
Since we don't want to create an individual page every time we write a blog post, we'll use a [dynamic route](https://docs.astro.build/en/core-concepts/routing/#dynamic-routes) for our blog posts.
...
- [Astro docs on content collections](https://docs.astro.build/en/guides/content-collections/)
- [Watch this tutorial as a video](https://youtu.be/gqz0d1GguTg)
