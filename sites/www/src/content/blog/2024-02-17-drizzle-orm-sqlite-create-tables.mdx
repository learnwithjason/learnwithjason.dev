...
In `package.json`, make the highlighted changes. The commands prefixed with `db:` let us create and update the database. More on how they work later in this tutorial.

```json title="package.json" mark={2-5,11-15}
{
	"engines": {
		"node": ">=20.6.0"
	},
...
After searching around, I figured out one way to get the SQLite tables created using Drizzle. There are two steps:

1. Generate a migration (the SQL query to create the tables as defined in your schema)
2. Push the changes (actually run the SQL query)

Both steps can be done using Drizzle's CLI helper, [Drizzle Kit](https://orm.drizzle.team/kit-docs/overview).

### Generate a SQLite migration using Drizzle Kit

```sh
npm run db:generate
```

Under the hood, this runs `drizzle-kit generate:sqlite` ([docs](https://orm.drizzle.team/kit-docs/commands#generate-migrations)), which reads the schema and creates a migration file. A migration file is a set of SQL queries that update the database to match the current schema from its previous state. The first migration creates the tables in the first place, since the previous state was _nothing_.

### Push the SQLite database changes using Drizzle Kit

Generating the migration doesn't actually change the database — it only creates the queries and stores them in a file. To update the database, we actually have to _run_ the migration.

```sh
npm run db:push
```

Under the hood, this runs `drizzle-kit push:sqlite` ([docs](https://orm.drizzle.team/kit-docs/commands#prototype--push)), which executes any migrations it finds to bring the database up to the correct current state. In this case, it will create the tables defined in `db/schema.ts` using the SQL queries that were generated in the previous step.
