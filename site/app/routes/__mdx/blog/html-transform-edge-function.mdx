...
tags: [edgefunctions, html, transform]
share:
  title: Replace Text & Images Using Edge Functions and HTMLRewriter
  text: >
    Transform HTML at request time using HTMLRewriter and Edge Functions. Update text, element attributes, and more.
...
In this article, we'll take a look at how to transform an HTML response using HTMLRewriter in a Netlify Edge Function.
...
CDNs have traditionally have only delivered static assets. With the introduction of Edge Functions, it's now possible to transform responses at the edge by distributing business logic closer to your users.
...
In order to keep responses fast, Edge Functions will stream (TKTK link to explainer) responses, which means that the response is sent to the browser in chunks instead of waiting until the full document is processed.
...
To solve for this, we can use the `HTMLRewriter` tool, which was originally [created by the Cloudflare team](https://developers.cloudflare.com/workers/runtime-apis/html-rewriter/) and later [ported to WASM by Florian Klampfer](https://deno.land/x/html_rewriter) for use with Deno and other open runtimes.
...
To see this site running in production, [check out the demo](https://html-transform-edge-functions.netlify.app/).
