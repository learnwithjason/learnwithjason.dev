...
Promises are extremely powerful for asynchronous operations in JavaScript (sometimes referred to as “Ajax”, as popularized by jQuery), and async functions make them easier to read and reason about. However, they also introduce some sneaky traps that can lead to slowdowns if we're not careful.
...
<figure style={{ margin: '0 -5vw', width: 'auto' }}>
...
async function getData() {
  ...
  ).then((response) => response.json()); // .then still works when it makes sense!
...
Promises run in parallel because they don’t care what happens outside of them — that‘s why we can only access their content inside a `.then`. This means that both `asyncThing1()` and `asyncThing2()` run at the same in our first example.
